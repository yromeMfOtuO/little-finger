...
def xingren_elk_read_func(path):
    return LogReaderV2.read_log_from_json_v2(
        path,
        lambda x: x['rawResponse']['hits']['hits'],
        lambda x: x['_source']['message'],
        LogReader.filter_method_by_contains('<--response start'), # 需要根据特定日志修改
        LogReader.reg_method_by_regex('body:(.*?),tookMs', 1), # 需要根据特定日志修改
        lambda x: json.loads(x)['dataList'],
        flatten
    )


def us_elk_read_func(path):
    return LogReaderV2.read_log_from_json_v2(
        path,
        lambda x: x['hits']['hits'],
        lambda x: x['_source']['log'],
...
def convert_dict_v2(path, key_column, json_read_func=xingren_elk_read_func) -> dict:
    """
    读取数据并转换为 dict
    :param path: 数据文件 path
    :param key_column: 作为key的列名
    :param json_read_func: json 数据读取方法
    :return:
    """
    data = {}
...
def compare(left_path: str, right_path: str, relate_column: List[str], compare_column: str) -> None:
    ...
    left_dict = convert_dict_v2(left_path, relate_column[0])
    right_dict = convert_dict_v2(right_path, relate_column[1])
